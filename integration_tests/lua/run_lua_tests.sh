...
nvim --headless -u NONE --noplugin \
  ...
  -c "lua
    local failures = 0
    for _, spec in ipairs({ 'spec_version', 'spec_platform', 'spec_download_url', 'spec_install', 'spec_setup', 'spec_download', 'spec_github_token', 'spec_health', 'spec_commands' }) do
