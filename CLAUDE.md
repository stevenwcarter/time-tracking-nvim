...
- `async_rt.rs` — a single lazily-initialized Tokio runtime shared by everything that calls into `time-tracking-cli`'s async API (the weekly view, opening today's file).
